"""
Code to create diverse dataset 
...
def get_diverse_data(df_slm, df_llm):
    ...
    # Concat 
    df_diverse = pd.concat([df_slm, df_llm_to_add])
    print("Shape after mixing LLM and SLM: ", df_diverse.shape)
    
    # Reset index and recreate 'id' column 
    df_diverse.drop(columns=['id'], inplace=True)
    df_diverse.reset_index(drop=False, inplace=True)
    df_diverse.rename(columns={'index':'id'}, inplace=True)
...
def run(config_path):
    ...
    dfs_diverse = {'cot':None, 'pot':None, 'l2m':None, 'combined':None, 'cot-biased':None, 'pot-biased':None, 'l2m-biased':None}

    # Get diverse Data 
    dfs_diverse['cot'] = get_diverse_data(dfs_slm['cot'], dfs_llm['cot'])
    dfs_diverse['pot'] = get_diverse_data(dfs_slm['pot'], dfs_llm['pot'])
    dfs_diverse['l2m'] = get_diverse_data(dfs_slm['l2m'], dfs_llm['l2m'])

    # Concat diverse Data 
    dfs_diverse['combined'] = pd.concat([dfs_diverse['cot'], dfs_diverse['pot'], dfs_diverse['l2m']], axis=0).reset_index(drop=True)
    
    # Biased Data 
    dfs_diverse['cot-biased'] = get_biased_data(dfs_diverse['combined'], 'cot')
    dfs_diverse['pot-biased'] = get_biased_data(dfs_diverse['combined'], 'pot')
    dfs_diverse['l2m-biased'] = get_biased_data(dfs_diverse['combined'], 'l2m')

    # Save all data
    for strategy, df in dfs_diverse.items():
        df.rename(columns={'predictions':'output_answer', 'answer':'correct_answer', 'predicted_num_answer':'llm_numeric_ans'}, inplace=True)
        df.to_csv(os.path.join(save_dir, 'df-diverse-{strat}-epoch{ep}.csv'.format(strat=strategy, ep=epoch)), index=False)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create diverse Data")
